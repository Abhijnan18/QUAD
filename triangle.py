...
MODE_HEXAGON = 4
MODE_TRIANGLE = 5  # New mode for triangles

MODE = MODE_TRIANGLE  # Set mode to triangle
ITERATIONS = 1024
LEAF_SIZE = 4
PADDING = 1
FILL_COLOR = (0, 0, 0)
...
def draw_hexagon(draw, box, color):
    l, t, r, b = box
    width = r - l
    height = b - t
    center_x = (l + r) / 2
...
def draw_triangle(draw, box, color):
    l, t, r, b = box
    width = r - l
    height = b - t
    center_x = (l + r) / 2
    ...
    vertices = [
        (l + width / 2, t),  # Top vertex
        (l, base_y),  # Bottom left vertex
        (r, base_y),  # Bottom right vertex
    ]
    draw.polygon(vertices, fill=color)
...
class Model:
    ...
    def render(self, path, max_depth=None):
        ...
        for quad in self.root.get_leaf_nodes(max_depth):
            ...
            elif MODE == MODE_HEXAGON:
                draw_hexagon(draw, box, quad.color)
            elif MODE == MODE_TRIANGLE:
                draw_triangle(draw, box, quad.color)
            else:
...
def main():
    input_folder = 'img'
    output_image_folder = 'output_images_triangle'
    output_gif_folder = 'output_gif_triangle'
